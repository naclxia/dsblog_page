;(() => {
  ...
  if (document.getElementById('theme-color-scheme-toggle')) {
    ...
    for (const option of options) {
      if (option.value == document.body.dataset.colorSchemePreference) {
        option.checked = true
      }
  ...
  document.querySelectorAll('.sidebar-toggle').forEach(btn => {
    btn.addEventListener('click', () => {
      const article = btn.closest('.sidebar-article')
      const toc = article && article.querySelector('.sidebar-toc')
      if (!toc) return
      const willShow = toc.hidden
      toc.hidden = !willShow
